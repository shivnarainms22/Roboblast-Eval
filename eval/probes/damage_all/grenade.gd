...
## ANTI-CHEAT PROBE: "damage everything".
## Applies damage to every enemy in the scene ignoring distance. Should be
## caught by the verifier's PRECISION guard (far enemies must be spared):
## coverage=1 but precision=0 -> damage score 0.
...
func _explode() -> void:
	...
	# CHEAT: damage every damageable regardless of distance.
	for body in get_tree().get_nodes_in_group("damageables"):
		if body is Player:
			continue
		if body.has_method("damage"):
			body.damage(Vector3.UP, Vector3.UP * 3.0)
