...
def video(name: str, caption: str) -> str:
    ...
    The GIFs above are inlined as data URIs so the page always reads standalone; the
    full recordings are ~29 MB and are referenced from the repo instead of embedded,
    which keeps the file openable rather than a 40 MB blob. Opened from a clone (as
    the repo intends), they play with no build step.
    """
...
<title>RoboBlast — an agent-coding eval task &amp; verifier</title>
<style>
...
<div class="wrap">
  ...
  <div class="card">
    <table style="margin:0">
      ...
      <tr><td><b>Agent run 1</b></td><td class="num bad">0</td><td class="num">55</td>
          <td class="num warn"><b>{S['run1']['total']:g}</b></td><td>crashes at runtime · swells</td></tr>
      ...
      <tr><td><b>Agent run 3</b></td><td class="num">35</td><td class="num">55</td>
          <td class="num warn"><b>{S['run3']['total']:g}</b></td><td>damage correct · swells</td></tr>
    </table>
  ...
  <p>That gives the grader a <b>deterministic backbone</b> (did the right enemies get hit? were the
     ...
     left intact, so the ablation is subtle: the grenade still "goes off", it just does nothing.</p>
  ...
  <h2><span class="n">2.</span>Before / after</h2>
  <div class="grid">
  ...
  <p class="muted">Both graded inside the game's real <code>playground.tscn</code> level — its own sky,
     glow, tonemap and lighting — not a synthetic scene. The blast is isolated by frame-differencing
     against a pre-detonation baseline, by <em>colour change</em>, so only pixels the blast actually
     alters are measured.</p>

  <h3>The same thing, played by a human</h3>
  <div class="grid">
    {video("original", "<b class='good'>Original.</b> Throw a grenade: the blast punches, enemies are flung.")}
    {video("ablated", "<b class='bad'>Ablated.</b> The grenade arcs, bounces, and <em>bangs</em> — and nothing happens. Enemies walk on. This is the game the agent is handed.")}
  </div>
  ...
  <div class="grid">
    ...
    <div>
      ...
      <table style="margin-top:6px">
        ...
        <tr><td>fiery hue</td><td class="num">8</td><td class="muted">orange/red</td></tr>
  ...
  <p>Every sub-behaviour is a <em>generic</em> property of an explosion tied to the spec's wording —
     not a copy of the original's implementation. <b>Blast size, ember count and effect structure are
     deliberately not scored</b>: they are implementation choices, and grading them would over-fit to
     the original. (run1's fireball covers <b>3.4×</b> more screen than run2's. Neither is penalised.)</p>
  ...
  <div class="callout good">
    <b>Determinism — and it was broken.</b> A review found the grader was <em>not</em> reproducible:
    ...
    and this was the single most important fix in the project.</span>
  </div>
...
./eval/run_probes.ps1   -Project ../abl                   # anti-cheat suite</code></pre>
  ...
  <p>The first rubric gave two of three agents a clean 100 — a red flag, and the brief is explicit
     that a strong agent must land <em>partial</em>. Rather than invent a gate, I measured the
     temporal brightness curve of every blast and found a real, generic discriminator hiding in it.</p>

  <div class="card">
    <h3 style="margin-top:4px">Time from onset to peak brightness</h3>
    {RISE_SVG}
    <p class="muted" style="margin-bottom:0">The original <b>flares</b> — full brightness in a single
      frame, like a detonation. Every agent <b>swells</b> like an inflating balloon over 12–16 frames.
      Same colour, same brightness, same cleanup. Wrong <em>attack</em>. All three lose the 10-point
      <code>flash</code> sub-score, and the margin is 3–4×, not a hair's breadth.</p>
  </div>
  ...
  <figure style="margin-top:20px"><img src="{COMPARISON}" alt="Frame-by-frame comparison strip of the original explosion against the ablated build and the three agent attempts.">
    <figcaption>Frame-by-frame, same moments after detonation. The original is at full intensity
  ...
  <div class="callout warn">
    ...
    solution</b> and changed <em>one number</em>: its expansion tween, <code>0.35s → 0.05s</code>.
    Nothing else. Rise went <b>12 frames → 1</b>, and the score went <b>90 → 100</b>. So the last 10
    points are winnable <em>by the agent's own code-driven design</em> — which is structurally nothing
    like the original's shader/mesh approach. The gate is real, reachable, and not a false-negative
    against a different implementation. The agents' miss is <b>a real defect in their work</b>, fixable
    with a single constant in their own file.
  </div>
  ...
  <p>All three wrote <b>structurally different</b> solutions — run1 a single 110-line script, run2 a
     script <em>plus a scene file</em>, run3 a 91-line script — and none of them used shaders, unlike the
     original. They also picked their own blast radius (<b>4.0 m</b> each; the original's is
     <b>3.0 m</b> — nobody recovered it). The verifier grades what happens on screen and to the
     enemies, so all three are judged fairly despite sharing no implementation with the original,
  ...
  <div class="callout warn">
    ...
    <b>could not see the defect by eye</b> — the blasts are bright, orange, centred, and clean up after
    themselves. The swell is a <em>temporal</em> property lasting a fraction of a second, and at full
    speed it simply reads as "a decent explosion".
    <p style="margin-bottom:0">That is the whole argument for grading the visual outcome
      ...
      failure the brief is hunting for. And the crash in §8 is worse still: it is invisible in normal
      play until you happen to blow up a crate first.</p>
  </div>
  ...
  <p>Every cheat is caught, and the best a fake manages is <b>87</b> — only a correct solution reaches
     100. Two of these were written <em>because</em> earlier probes found real holes: a screen-filling
     orange flash once scored <b>91</b>, which is what forced the <b>compactness</b> gate (a blast that
     fills the screen is not a localized blast, so it forfeits the whole visual score). The
     <code>small_radius</code> entry is <b>not</b> a cheat — it is an honest, under-built solution
  ...
  <div class="grid">
    <div class="card">
      ...
      <p style="margin-bottom:0">Each rollout ran in a <code>git archive</code> extract of the ablated
        branch: <b>history depth 1</b>, no <code>main</code>, <b>no remotes</b>, and no verifier
        anywhere on disk. The original is unreachable via branches, history, or the grader.</p>
    </div>
    <div class="card">
      <h3 style="margin-top:0">The tools</h3>
      <p style="margin-bottom:0">The game is public on GitHub, so obscurity is worthless — integrity
        has to be enforced at the tool layer. <code>Bash</code>, <code>WebFetch</code> and
  ...
  <div class="callout good">
    ...
    attempted a <code>Bash</code> call</b> and were blocked: <code>Error: No such tool available: Bash</code>.
    The lockdown is enforced in the wild, not merely configured. Their solutions are also
    <b>code-driven</b> while the original is <b>shader-driven</b> — a cheat would have reproduced the
    shaders. None did, and none recovered the real 3.0 m radius.
  </div>
  ...
  <div class="callout bad">
    <b>① Real agent defects — the verifier is right.</b>
    <p><b>The crash (run1, run2).</b> Both iterate the <code>damageables</code> group and call
      ...
      node with no such method. Result: <code>SCRIPT ERROR: Nonexistent function 'damage'</code>, in
      normal gameplay, whenever a grenade goes off near a broken crate. run3 guards with
      <code>has_method()</code> and is correct.</p>
    <p style="margin-bottom:0"><b>The swell (all three).</b> Measured, generic, and fixable with one
      constant — see §4.</p>
  </div>

  <div class="callout warn">
    <b>② Real verifier artefacts — the grader was wrong, and I fixed it.</b>
    <p><b>Non-determinism.</b> Scores drifted with machine speed (§3). Found by review, not by me.</p>
    <p style="margin-bottom:0"><b>A threshold in the wrong place.</b> The <code>hue</code> gate sat at
      0.6 — <em>inside</em> the cluster of correct solutions (they measure 0.57–0.96), so 3 points were
      a coin-flip for whichever blast happened to be smallest. Worse, a warm-<em>pixel-fraction</em> is
      size-biased, so it was quietly grading <b>blast size</b> — an implementation choice — rather than
      <b>colour</b>. Moving the boundary into the empty gap between the two populations (fiery 0.57–0.96,
      blue 0.00) took the worst-case margin from <b>+9% to +79%</b> and <b>changed no score</b>. That
      score-neutrality is the tell that it was a genuine robustness fix and not a thumb on the scale.</p>
  </div>

  <div class="callout">
    <b>③ Neither — an incomplete spec.</b>
    <p style="margin-bottom:0">The Player is <em>also</em> in <code>damageables</code>, and <b>run2
      damages you with your own grenade</b>. The original doesn't. But my spec says <em>"it damages
      nearby <b>enemies</b>"</em> and is <b>silent on the player</b> — so an agent that hurts the player
      has satisfied every word it was given. Docking run2 for it would be grading an <b>unstated
      requirement</b>: exactly the false-negative failure the brief warns against hardest. So the harness
      <b>measures and reports</b> it (<code>player_damaged_DIAGNOSTIC_NOT_SCORED</code>) and
  ...
  <div class="callout bad">
    ...
    only enemies; my visual harness <em>stripped the crates out</em>. I had even written that narrowness
    down as an accepted limitation. It was hiding a gameplay-breaking bug in <b>two of three</b>
  ...
  <ul>
    <li><b>"Headless" has a caveat.</b> The damage half is true <code>--headless</code>. The visual half
      needs a real render context — Godot's dummy renderer produces no pixels. It runs fully automated
      with no human, but on a headless CI it would need a virtual display.</li>
    <li><b>Cross-hardware reproducibility is untested.</b> Determinism is demonstrated on one
      machine/GPU/Godot build. The margins are wide (the flash discriminator has 3–4× headroom, hue +79%),
    ...
    <li><b>The verifier cannot rank two <em>equally good</em> attempts.</b> run1 and run2 tie at 55 with
      ...
      (correct / missing / cheat / broken), not shades of polish.</li>
    <li><b>Damage is tested against beetles and crate debris</b> — not bee-bots or intact crates. Better
      than it was (that gap hid the crash), but still not the full cast.</li>
  </ul>
  ...
  <div class="foot">
    ...
    <code>agent/run1–3</code> are the rollouts, verbatim · <code>eval</code> holds the verifier, the probes and the results.
